...
// We declare the 'start' label as global (accessible from outside this file), since the linker will need to know where
// it is. In a bit, we'll actually take a look at the code that defines this label.
.global start
...
.set MB_MAGIC, 0x1BADB002          // This is a 'magic' constant the GRUB will use to detect our kernel's location
.set MB_FLAGS, (1 << 0) | (1 << 1) // This tells GRUB to 1: load modules on page boundaries and 2: provide a memory map
// (this is useful later in development)

// Finally, we calculate a checksum that includes all the previous values
.set MB_CHECKSUM, (0 - (MB_MAGIC + MB_FLAGS))

// We now start the section of the executeable that will contain out Multiboot header
.section .multiboot
...
.section .text
    ...
    start:
        ...
        // If, by some mysterious circumstances, the kernel's C code never returns, all we want to do is to hang the CPU
        hang:
