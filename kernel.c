...
void term_init()
{
    ...
    for (int col = 0; col < VGA_COLS; col++)
    {
        ...
        {
            // The VGA textmode buffer has a size (VGA_COLS * VGA_ROWS)
...
void term_putc(char c)
{
    ...
    if (term_row >= VGA_ROWS)
    {
        term_col = 0;
        term_row = 0;
        // TODO: create an history buffer, whenver terminal is full put top row in buffer, move all rows one row up and fill last row with blanks (scrolling)
    }
}
